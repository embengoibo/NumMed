...
double bessI1(double x) {
	double ax;
	double y, ans;
	if (fabs(x) < 3.75) {  //Approximation ber Polynom
		y = pow((x/3.75), 2);
		ans = x*(0.5 + y*(0.87890594+y*(0.51498869 +y*(0.15084934
			+y*(0.2658733e-1+y*(0.301532e-2+y*0.32411e-3))))));

		return ans;
	} else {    //Approximation ber Polynom
		ax = fabs(x);
		y = 3.75/ax;
...
double bessY1(double x) {
	...
	if (x < 8.0) {  //Approximation ber Polynom
		y = pow(x, 2);
		...
		return ans;
	} else {    //Approximation ber cos sin Summe
		z = 8.0/x;
		y = pow(z, 2);
		xx = x-2.356194491;
...
double bessY0(double x) {
	double xx, z;
	double y, ans, ans1, ans2;
	if (fabs(x) < 8.0) {  //Approximation ber Polynom
		y = pow(x, 2);
...
double bessJ1(double x) {
	...
	if (fabs(x) < 8.0) {               //Approximation durch Polynom
		y = pow(x, 2);
...
// x        : Argument |x| < 1
//-------------------------------------------------------------------------
double elip_K(double k) {  //Berechnung nach Reihendarstellung Bronstein Seite 124
