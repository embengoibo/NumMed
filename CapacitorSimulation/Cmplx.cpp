			...
			Uebergebene Matrix wird nicht veraendert  */
{
	...
	if (ma != NULL) { // Uebernehmen der neuen Matrix
		...
		for (jSol = 0; jSol < nSol; jSol++) {
			for (iZeil = 0; iZeil < dim; iZeil++) { //Uebernahme der Erregung
		...
		if (gauss(ma, dim, nSol)) {
			for (jSol = 0; jSol < nSol; jSol++) {
				for (iZeil = 0; iZeil < dim; iZeil++) { //Uebernahme des Ergebnisses
